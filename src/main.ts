...
const setSRGB = (t: THREE.Texture) => {
  // three r160+
  (t as any).colorSpace = (THREE as any).SRGBColorSpace || (THREE as any).SRGBColorSpace;
  return t;
};

/* ------------ ユーティリティ ------------ */
function disposeChildren(g: THREE.Group) {
...
function buildMall() {
  ...
  function makeBooth(name: string, side: "L" | "R", z: number, imgs: string[]) {
    ...
    // U字腰壁（通路側：低い前パラペット、両サイド：ラウンド角風に少し内側寄せ）
    const lowH = 0.9;
    ...
    sign.position.set(parX + (side === "L" ? 0.01 : -0.01), signH, z);
